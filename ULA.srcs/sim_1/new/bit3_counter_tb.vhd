...
begin
    ...
    cover_track : process (sysclk) is
    begin

        if rising_edge(sysclk) then
            if (out_uut = "000") then
                states_visited(0) <= '1';
            elsif (out_uut = "001") then
                states_visited(1) <= '1';
            elsif (out_uut = "010") then
                states_visited(2) <= '1';
            elsif (out_uut = "011") then
                states_visited(3) <= '1';
            elsif (out_uut = "100") then
                states_visited(4) <= '1';
            elsif (out_uut = "101") then
                states_visited(5) <= '1';
            elsif (out_uut = "110") then
                states_visited(6) <= '1';
            end if;
        end if;

    end process cover_track;

    -- End-of-sim coverage report. Warns (not errors) on any missed state.
    report_cov : process (sim_done) is
    begin
        ...
        if (sim_done) then
            ...
            report "bit3_counter_tb: simulation complete."
                severity note;
        end if;

    end process report_cov;
