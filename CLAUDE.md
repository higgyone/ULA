...
- `master_horiz_counter` now instantiates `(T_Structure)` for schematic fidelity. **✅ VERIFIED in xsim** (eyeballed `master_horiz_counter_tb`, run 10 us): C0–C5 count 0→63, C6–C8 count 0→6 then wrap, full line 0→447, `hc_rst` high across the C6–C8="110" window. Integration correct.
  ...
  - **`master_horiz_counter_tb` is now self-checking.** A TB-internal `golden_count` (0→447) increments once per rising `clk7` (clears with reset); a checker samples the 9-bit tap concat `c8..c0` (= `c_upper*64 + c_lower`) on the falling `clk7` edge (mid-period, settled) and asserts `taps = golden_count`, with a "line complete" heartbeat each wrap. Still `run 10 us` (≈2 lines) — the default `run 1000ns` only does ~100 cycles and never advances C6–C8. Pass = no `MHC count mismatch`, with `line complete` notes appearing.
