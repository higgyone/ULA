...
- `master_horiz_counter` now instantiates `(T_Structure)` for schematic fidelity. **✅ VERIFIED in xsim** (eyeballed `master_horiz_counter_tb`, run 10 us): C0–C5 count 0→63, C6–C8 count 0→6 then wrap, full line 0→447, `hc_rst` high across the C6–C8="110" window. Integration correct.
  ...
  - **Run-length note:** `master_horiz_counter_tb` is stimulus-only (no asserts). Default `run 1000ns` only does ~100 clk7 cycles — C6–C8 never advance; you must `run 10 us` to exercise the T_Structure stage. Tip for reading the count: make a virtual bus or add `c_lower <= c5&c4&c3&c2&c1&c0` (0–63) / `c_upper <= c8&c7&c6` (0–6) in the TB. Optional future improvement: TB-internal golden integer `full_count` (0→447) asserting `c8..c0` each line, like `bit3_counter_tb`.
